...
#pragma region 서식지정자
	...
	printf("data의 값: %d\n", data);
	// float 4byte 크기의 실수형 자료형은 소수점 이하 6자리까지만 표기합니다.
	printf("decimal의 값 : %f\n", decimal);
...
#pragma region 비트(bit)
	// 데이터를 나타내는 최소의 단위이며, 0 또는 1의 조합으로 논리계산을 수행하는 단위. 
	// bit가 8개 모이면 1byte가 된다.

	// 비트 연산자 : 비트 단위로 논리 연산을 수행하기 위해 사용하는 연산자입니다.
